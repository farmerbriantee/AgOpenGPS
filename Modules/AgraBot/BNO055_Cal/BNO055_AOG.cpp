...
void BNO055::getCalibration(uint8_t* sys, uint8_t* gyro, uint8_t* accel, uint8_t* mag) {
  uint8_t calData = readByte(BNO055_ADDRESS, BNO055_CALIB_STAT);
  if (sys != NULL) {
    *sys = (calData >> 6) & 0x03;
  }
  if (gyro != NULL) {
...
bool BNO055::getSensorOffsets(bno055_offsets_t &offsets_type)
{
    if (isFullyCalibrated())
    {
        ...
        offsets_type.accel_offset_x = (readByte(BNO055_ADDRESS, BNO055_ACC_OFFSET_X_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_ACC_OFFSET_X_LSB));
        offsets_type.accel_offset_y = (readByte(BNO055_ADDRESS, BNO055_ACC_OFFSET_Y_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_ACC_OFFSET_Y_LSB));
        offsets_type.accel_offset_z = (readByte(BNO055_ADDRESS, BNO055_ACC_OFFSET_Z_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_ACC_OFFSET_Z_LSB));

        offsets_type.gyro_offset_x = (readByte(BNO055_ADDRESS, BNO055_GYR_OFFSET_X_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_GYR_OFFSET_X_LSB));
        offsets_type.gyro_offset_y = (readByte(BNO055_ADDRESS, BNO055_GYR_OFFSET_Y_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_GYR_OFFSET_Y_LSB));
        offsets_type.gyro_offset_z = (readByte(BNO055_ADDRESS, BNO055_GYR_OFFSET_Z_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_GYR_OFFSET_Z_LSB));

        offsets_type.mag_offset_x = (readByte(BNO055_ADDRESS, BNO055_MAG_OFFSET_X_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_MAG_OFFSET_X_LSB));
        offsets_type.mag_offset_y = (readByte(BNO055_ADDRESS, BNO055_MAG_OFFSET_Y_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_MAG_OFFSET_Y_LSB));
        offsets_type.mag_offset_z = (readByte(BNO055_ADDRESS, BNO055_MAG_OFFSET_Z_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_MAG_OFFSET_Z_LSB));

        offsets_type.accel_radius = (readByte(BNO055_ADDRESS, BNO055_ACC_RADIUS_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_ACC_RADIUS_LSB));
        offsets_type.mag_radius =   (readByte(BNO055_ADDRESS, BNO055_MAG_RADIUS_MSB) << 8) | (readByte(BNO055_ADDRESS, BNO055_MAG_RADIUS_LSB));
...
void BNO055::readIMU(){
    uint8_t rawData[4];  // x/y/z gyro register data stored here
    readBytes(BNO055_ADDRESS, BNO055_EUL_HEADING_LSB+2, 6, &rawData[0]);  // Read the six raw data registers sequentially into data array
    euler.roll = ((int16_t)rawData[1] << 8) | rawData[0] ;  
    euler.pitch = ((int16_t)rawData[3] << 8) | rawData[2] ;

	//read the vehicle turning around gyro
	readBytes(BNO055_ADDRESS, BNO055_GYR_DATA_X_LSB + 4, 6, &rawData[4]);							   
	euler.angVel = ((int16_t)rawData[5] << 8) | rawData[4];
