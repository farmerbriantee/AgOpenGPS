...
namespace AgOpenGPS
{
    public enum SkipMode { Normal, Alternative, considerWorkedTracks };
    public class CYouTurn
    {
        ...
        private int getNextNotWorkedLane(bool isTurnLeft, int rowSkipsWidth, bool isAB)
        {
            ...
            if (isAB)
            {
                ...
                while (mf.trk.gArr[mf.trk.idx].workedTracks.Contains(goalLine))
                {
                    rowSkipsWidth++;
                    if ((isTurnLeft && !mf.ABLine.isHeadingSameWay) || (!isTurnLeft && mf.ABLine.isHeadingSameWay))
                        goalLine++;
        ...
        public bool BuildCurveDubinsYouTurn(bool isTurnLeft, vec3 pivotPos)
        {
            ...
            if (skip_mode == SkipMode.considerWorkedTracks)
