  ...
  int pwmDrive = 0, drive = 0;;
  float pValue = 0, iValue = 0, dValue = 0;
 
  bool isHeaderFound = false, isDataReady = false;
  ...
  int   maxErr = 3000; //anti windup max
...
void loop()
{
  ...
  if (Serial.available()> 6 & isHeaderFound)
  {
        ...
        if (pidSetting == 0) {}
          ...
          switch(pidSetting){
            case 3:{ Kp +=0.1; break; }
            case 2:{ Kp -=0.1; break; }
            case 5:{ Ki +=0.001; break; }
            case 4:{ Ki -=0.001; break; }    
