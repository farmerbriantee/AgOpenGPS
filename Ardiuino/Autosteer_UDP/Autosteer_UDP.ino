   ...
   * 
   * 1660 minus 127 = 1533. 66.4 counts per 0.1volt
   * The 127 is so you can take WAS zero of 1660 and add or subtract 127.
   * 
   * Leave at 1533 - Only change if using a factory OEM wheel angle Sensor
   ...
   * 66.4 counts per volt = 199.2counts. 1660 - 199 - 127 = 1334.
   * So your new WAS_ZERO is 1334.
   */
  #define WAS_ZERO 1533  
  ...
  byte previous = 0;
   ...
   struct Storage {
      float Ko = 5.0f;  //overall gain
      float Kp = 5.0f;  //proportional gain
      float deadZone = 0.0f;  //integral gain
      float Kd = 0.0f;  //derivative gain 
      float steeringPositionZero = 1660.0;
      byte minPWMValue=10;
      int maxIntegralValue=150;//max PWM value for integral PID component
   ...
   struct Setup {
      ...
      byte isRelayActiveHigh = 0;
      
  };  Setup aogSettings;   //12 bytes
  ...
  */  
    ...
    */
    ...
    // Send Data to AOG
      ...
      Serial.print(aogSettings.CytronDriver); //The actual steering angle in degrees
      Serial.print(",");
        
      Serial.println(aogSettings.isRelayActiveHigh); 
      */
...
void udpSteerRecv(uint16_t dest_port, uint8_t src_ip[IP_LEN], uint16_t src_port, byte *data, uint16_t len)
{
 ...
 if (data[0] == 0x7F && data[1] == 0xFB) //Arduino Settings
  ...
  if (data[0] == 0x7F && data[1] == 0xFC)
  {
    ...
    steerSettings.maxIntegralValue = data[8]; //
