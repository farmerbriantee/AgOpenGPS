...
void loop()
{
  ...
  if (Serial.available() > 7 && isSettingFound)
  {
    ...
    steerSettings.deadZone = (float)Serial.read() * 0.01;   // read deadZone from AgOpenGPS
    ...
    steerSettings.steeringPositionZero = WAS_ZERO + Serial.read();  //read steering zero offset
